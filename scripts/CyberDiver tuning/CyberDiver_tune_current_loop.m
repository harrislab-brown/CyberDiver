function CyberDiver_tune_step_response()
...
try
    ...
    while tuning
        ...
        % get the gains
        fprintf(s, 'powerboard config get gains\n');
        response = fgets(s);
        if startsWith(response, 'ack')
        ...
        if input('Enter new gains? [y/n]: ', 's') == 'y'
            ...
            fprintf(s, sprintf('powerboard config set gains %f %f %f %f\n', Kp, Ki, Kd, Kff));
            response = fgets(s);
            if ~startsWith(response, 'ack')
                throw(MException('CyberDiver:serialError', 'Unexpected response from serial port.'));
            end
            ...
            if input('Perform step response? [y/n]: ', 's') == 'y'
                ...
                amplitude = input('Step amplitude (A): ');
                ...
                fprintf(s, 'controller set mode current_control\n');
                response = fgets(s);
                if ~startsWith(response, 'ack')
                    throw(MException('CyberDiver:serialError', 'Unexpected response from serial port.'));
                end
                ...
                plot(time_s, current_a, 'b-', 'DisplayName', 'Measured')
                legend show
                subplot(2,1,2)
                hold on
                box on
