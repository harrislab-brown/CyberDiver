...
void LEDSequence_Update(LEDSequence* led)
{
	...
	if (led->burst_sequence_array != NULL)
	{
		if (*(led->time_micros_ptr) - led->time_last_burst > led->burst_sequence_array[led->burst_sequence_index])
		{
			...
			if(++led->burst_sequence_index == led->burst_sequence_len)  /* increment the index */
				led->burst_sequence_array = NULL;
		}
	}

	/* set the LED state */
	if (LEDSequence_GetIsRunning(led))
	{
		if (led->burst_sequence_array != NULL)
		{
...
void LEDSequence_Stop(LEDSequence* led)
{
	/* stop the sequences */
...
void LEDSequence_SetState(LEDSequence* led, uint8_t state)
{
	/* control the LED manually */
	HAL_GPIO_WritePin(led->GPIO_Port, led->GPIO_Pin, state);
}

uint8_t LEDSequence_GetIsRunning(LEDSequence* led)
{
	return ((led->blink_sequence_array != NULL) || (led->burst_sequence_array != NULL));
}
